...
export default function CmsComments() {
  ...
  const filtered = useMemo(() => {
    if (tab === "all") return state.comments;
    return state.comments.filter(c => c.status === tab);
  }, [state.comments, tab]);
  ...
  const updateStatus = (id: string, status: "approved" | "spam") => {
    dispatch({ type: "UPDATE_COMMENT", payload: { id, status } });
    addActivity(status === "approved" ? "Approved comment" : "Marked as spam", "");
    toast.success(status === "approved" ? "Comment approved" : "Marked as spam");
  };

  const handleDelete = (id: string) => {
    dispatch({ type: "DELETE_COMMENT", payload: id });
    addActivity("Deleted comment", "");
    toast.success("Comment deleted");
    setDeleteId(null);
  };

  const handleBulk = (action: "approved" | "spam" | "delete") => {
    const ids = Array.from(selected);
    if (!ids.length) return;
    if (action === "delete") {
      dispatch({ type: "BULK_DELETE_COMMENTS", payload: ids });
      toast.success(`${ids.length} comments deleted`);
    } else {
      dispatch({ type: "BULK_UPDATE_COMMENTS", payload: { ids, status: action } });
      toast.success(`${ids.length} comments ${action}`);
    }
    setSelected(new Set());
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {tabs.map(t => (
          <button key={t} onClick={() => { setTab(t); setSelected(new Set()); }}
            ...
            {t} ({t === "all" ? state.comments.length : state.comments.filter(c => c.status === t).length})
          </button>
        ))}
      </div>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm" style={{ background: "rgba(245, 158, 11, 0.08)" }}>
          ...
          <button onClick={() => handleBulk("approved")} className="px-2.5 py-1 rounded text-xs font-medium" style={{ background: "rgba(34, 197, 94, 0.15)", color: "#22c55e" }}>Approve</button>
          <button onClick={() => handleBulk("spam")} className="px-2.5 py-1 rounded text-xs font-medium" style={{ background: "rgba(234, 179, 8, 0.15)", color: "#eab308" }}>Spam</button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: "1px solid #2a2d3e" }}>
              <th className="w-10 py-3 px-3"><input type="checkbox" checked={filtered.length > 0 && selected.size === filtered.length} onChange={toggleAll} aria-label="Select all" /></th>
            ...
            {filtered.map(c => {
              ...
              return (
                <tr key={c.id} className="transition-colors hover:bg-white/[0.02]" style={{ borderBottom: "1px solid #2a2d3e22" }}>
                  <td className="py-2.5 px-3"><input type="checkbox" checked={selected.has(c.id)} onChange={() => toggle(c.id)} /></td>
                  <td className="py-2.5 px-3 font-medium">{c.author}</td>
                  <td className="py-2.5 px-3 max-w-[200px] truncate" style={{ color: "#9ca3af" }}>{c.content}</td>
                  <td className="py-2.5 px-3 max-w-[150px] truncate" style={{ color: "#9ca3af" }}>{c.postTitle}</td>
                  <td className="py-2.5 px-3" style={{ color: "#9ca3af" }}>{c.date}</td>
                  <td className="py-2.5 px-3">
                    <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize" style={{ background: sc.bg, color: sc.text }}>{c.status}</span>
                  </td>
                  <td className="py-2.5 px-3">
