...
export default function CmsSettings() {
  ...
  const Field = ({ label, value, onChange, textarea }: { label: string; value: string; onChange: (v: string) => void; textarea?: boolean }) => (
    <div>
      ...
      {textarea ? (
        <textarea value={value} onChange={e => onChange(e.target.value)} rows={3} className={`${inputStyle} resize-none`} style={inputBg} />
      ) : (
        <input value={value} onChange={e => onChange(e.target.value)} className={inputStyle} style={inputBg} />
      )}
    </div>
  );
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="rounded-xl p-5 space-y-4" style={cardStyle}>
        ...
        <Field label="Blog Title" value={s.blogTitle} onChange={v => setS({ ...s, blogTitle: v })} />
        <Field label="Tagline" value={s.tagline} onChange={v => setS({ ...s, tagline: v })} />
        <Field label="Description" value={s.description} onChange={v => setS({ ...s, description: v })} textarea />
        <div>
